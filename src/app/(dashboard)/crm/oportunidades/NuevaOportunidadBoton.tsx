...
import { Plus, X, Briefcase, DollarSign, User, Loader2 } from 'lucide-react';
import { createOportunidad } from '@/app/actions/crm';
...
export default function NuevaOportunidadBoton({ clientes }: { clientes: Cliente[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await createOportunidad(formData);
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  {clientes.length === 0 && (
                    <p className="text-xs text-orange-600 mt-1">
                      Debes registrar al menos un cliente antes de crear oportunidades.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 mt-6">
                ...
                >
                  {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Guardar Oportunidad'}
                </button>
              </div>
            </form>
          </div>
