...
export default async function Home() {
  ...
  const stats = [
    { label: "Proyectos Activos", value: proyectosActivos.toString(), icon: <TrendingUp className="w-6 h-6 text-primary-light" />, trend: "En progreso" },
    { label: "Tareas Pendientes", value: tareasPendientes.toString(), icon: <CheckCircle className="w-6 h-6 text-orange-500" />, trend: "Requieren atención" },
    { label: "Ingresos del Mes", value: formatCurrency(ingresosMes), icon: <TrendingUp className="w-6 h-6 text-success" />, trend: "Mes actual" },
    { label: "Movimientos", value: movimientosMes.length.toString(), icon: <Clock className="w-6 h-6 text-purple-500" />, trend: "Registrados este mes" },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-surface rounded-2xl p-6 card-shadow border border-slate-100">
