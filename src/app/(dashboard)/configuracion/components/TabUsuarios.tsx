...
import { Plus, Edit, User, Mail, Shield, Crown } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {usuarios.map(usuario => {
          ...
          return (
            <div key={usuario.id} className={`bg-white p-5 rounded-2xl border ${isSuper ? 'border-amber-200 shadow-amber-100/50' : 'border-slate-100'} shadow-sm flex flex-col gap-4 relative group hover:border-primary/20 transition-colors`}>
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shrink-0 ${isSuper ? 'bg-gradient-to-br from-amber-400 to-amber-600 text-white border-0' : 'bg-slate-100 border border-slate-200 text-slate-400'}`}>
                  {isSuper ? <Crown className="w-6 h-6" /> : (usuario.nombre ? usuario.nombre.charAt(0).toUpperCase() : <User className="w-6 h-6" />)}
                </div>
                <div className="overflow-hidden flex-1">
                  <h3 className="font-bold text-slate-800 truncate pr-8">{usuario.nombre || 'Sin nombre'}</h3>
